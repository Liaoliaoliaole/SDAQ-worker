...
const int w_stat_info_width = 35;
const int w_meas_height = 19;
const int w_meas_width = 35;
const int w_spacing = 2;
const int term_width = w_meas_width + w_stat_info_width + w_spacing * 3;
const int term_height = w_meas_height + w_stat_info_height + 5;
...
//local functions
int w_init(struct thread_arguments_passer *arg);
...
int Measure(int socket_num, unsigned char dev_addr, opt_flags usr_flag)
{
	...
	printf("\e[8;%d;%dt",term_height,term_width);//resize terminal window to the application's needs
	...
	while(running>0)
	{
		...
		switch(user_pressed_key)
		{
			...
			case 'q': case  3 : running=0; break; //SIGINT or Ctrl+c
...
int w_init(struct thread_arguments_passer *arg)
{
	...
	if(term_row >= term_height && term_col >= term_width) 
	{
		arg->status_win  = newwin(w_stat_info_height,w_stat_info_width, 2, term_col/2-w_stat_info_width-w_spacing/2);
		arg->info_win    = newwin(w_stat_info_height,w_stat_info_width, 2, term_col/2+w_spacing/2);
		arg->meas_win    = newwin(w_meas_height,w_meas_width, 2+w_stat_info_height, term_col/2-w_meas_width-w_spacing/2);
		arg->raw_meas_win= newwin(w_meas_height,w_meas_width, 2+w_stat_info_height, term_col/2+w_spacing/2);
		scrollok(arg->status_win, TRUE);
		scrollok(arg->info_win, TRUE);
		scrollok(arg->meas_win, TRUE);
		scrollok(arg->raw_meas_win, TRUE);
		clear();
		mvprintw(1,term_col/2-8,"Device Address: %d",arg->dev_addr);
		mvprintw(term_height-2,term_col/2-w_stat_info_width,"Function Buttons:");
		mvprintw(term_height-1,term_col/2-w_stat_info_width,"'q' Quit 1 Start 2 Stop 3 Dev_Info R Un-Calibrated S Sync");
		refresh();
				...
				pthread_mutex_lock(&display_access);
					switch(id_dec->payload_type)
					{
						...
						case Device_status: 
							...
							mvwprintw(arg->status_win,2,3,"Type = %s",dev_type_str[status_dec->dev_type]);
							mvwprintw(arg->status_win,3,3,"Serial number = %d",status_dec->dev_sn);
							mvwprintw(arg->status_win,4,3,"State : %9s",status_byte_dec(status_dec->status,Run_Standby));								
							mvwprintw(arg->status_win,5,3,"IsSync?  : %3s",status_byte_dec(status_dec->status,In_sync));
							mvwprintw(arg->status_win,6,3,"Error?   : %3s",status_byte_dec(status_dec->status,Error));
